package by.epam.lab;

public class Constants {
    public final static String DELIMITER = ";";
    public final static String BYN_TO_STRING = "%d.%02d";
    public final static int BYN_VALUE_IN_KOPS = 100;
    public final static String PATH = "src/by/epam/lab/in.csv";
    public final static int NUMBER_OF_PURCHASE_INDEXES = 3;
    public final static int NAME_INDEX = 0;
    public final static int NUMBER_INDEX = 2;
    public final static int PRICE_INDEX = 1;
    public final static int DISCOUNT_INDEX = 3;
    public final static String LAST_PURCHASE_MAP = "Last purchase map";
    public final static String FIRST_PURCHASE_MAP = "First purchase map";
    public final static String ENUMERATED_MAP = "Enumerated map";
    public final static String OUTPUT_AFTER_DELETING = "\nAfter deleting";
    public final static String OUTPUT_COST_EACH_DAY ="total cost for each day: \n";
    public final static int HASH_CODE_CONST = 31;
    public final static String REQUIRED_PURCHASE = "required purchase ";
    public final static String NOT_FOUND = "not found ";
    public final static String MEAT = "meat";
    public final static String MILK = "milk";
    public final static String BREAD = "bread";
    public final static String MONDAY_PURCHASES = "Monday purchases: ";
    public final static String DAY = " day: ";
    public final static String DAY_PURCHASES_MAP = "Day purchases map";
    public final static String FIND_FIRST_DAY ="First day for purchase ";
    public final static String FIND_LAST_DAY = "Last day for purchase";
    public final static String FIND_FIRST_DAY_FAILED ="First day for purchase";
    public final static String OPEN_BRAСKET = " (";
    public final static String CLOSE_BRAСKET = ") ";
}

